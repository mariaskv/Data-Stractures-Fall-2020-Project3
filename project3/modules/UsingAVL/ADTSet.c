...
static void visit_help(Set set, SetNode node, VisitFunc visit){
	if(node->left != NULL){
		visit_help(set,node->left,visit);
		//find_min(set,node->left,visit);
		visit(set_node_value(set,node));
	}
	else{
		visit(set_node_value(set,node));
	...
	if(node->right != NULL){
		visit_help(set,node->right,visit);
		return;
	}
}

void set_visit(Set set, VisitFunc vist){
	visit_help(set,set->root,vist);
}
