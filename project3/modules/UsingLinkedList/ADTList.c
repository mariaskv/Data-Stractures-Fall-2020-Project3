...
void list_remove(List list, ListNode node) {
	if(node==NULL)	
		...
		ListNode prev;
		for(ListNode temp=list->dummy;temp!=node;temp=list_next(list,temp))//starting with the first element of the list
			prev=temp;		//move on until the element before the given
		list_remove_next(list,prev); //using list_remove_next remove the element
	}
}
